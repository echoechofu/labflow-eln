...
const requiredTables = ['experiments','tasks','protocols','protocol_versions','records','record_changes','samples','sample_relations','record_samples','attachments','results']

export function validateDatabase(databasePath: string) {
  const db = new Database(databasePath, { readonly: true })
  try {
    if ((db.pragma('integrity_check', { simple: true }) as string) !== 'ok') throw new Error('SQLite integrity_check failed')
...
export function openDatabase(databasePath: string) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS protocols (id TEXT PRIMARY KEY, name TEXT NOT NULL, category TEXT NOT NULL, active_version INTEGER NOT NULL, accent TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS protocol_versions (protocol_id TEXT NOT NULL REFERENCES protocols(id), version_number INTEGER NOT NULL, schema_json TEXT NOT NULL, PRIMARY KEY(protocol_id, version_number));
    ...
    CREATE TABLE IF NOT EXISTS samples (id TEXT PRIMARY KEY, workspace_id TEXT NOT NULL DEFAULT 'local', experiment_id TEXT NOT NULL REFERENCES experiments(id), sample_code TEXT NOT NULL, sample_type TEXT NOT NULL, source_record_id TEXT REFERENCES records(id), parent_sample_id TEXT, UNIQUE(workspace_id, sample_code));
...
export function databaseExistsOutsideProject(databasePath: string, projectRoot: string) {
  return existsSync(databasePath) && !databasePath.startsWith(`${projectRoot}/`)
}
